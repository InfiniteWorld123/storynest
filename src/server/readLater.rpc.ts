...
import { storySave } from '#/db/schema'
...
// NOTE: These are client-callable RPCs. They intentionally live in a non-`.server.ts`
// module to satisfy TanStack Start import-protection, while keeping DB access in the handler.

export const getReadLaterCountRpc = createServerFn({ method: 'GET' })
...
export const saveReadLaterRpc = createServerFn({ method: 'POST' })
  .middleware([authMiddleware])
  .inputValidator(saveReadLaterSchema)
  .handler(
    catchAsyncFn(async ({ data, context }): ServerOk<ReadLaterItem> => {
...
export const removeReadLaterRpc = createServerFn({ method: 'POST' })
  .middleware([authMiddleware])
...
export const getReadLaterStoriesRpc = createServerFn({ method: 'GET' })
  .middleware([authMiddleware])
